...
func TestRouter(t *testing.T) {
	...
	res, err = http.Post(ts.URL+"/helloworld", "text/plain", nil)
	if err != nil {
		t.Fatal(err)
	}
	if res.StatusCode != http.StatusMethodNotAllowed {
		t.Errorf("Status code for /helloworld is wrong. Have: %d, want: %d.", res.StatusCode, http.StatusMethodNotAllowed)
	}
